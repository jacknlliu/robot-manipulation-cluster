...
LOCAL_HOST_IP="192.168.1.121"

PARTNER_IP_ADDRs="192.168.1.123"

MYSELF_CONTAINER_IP="10.2.2.1"
...
CONTAINER_NAME="ros_kinetic_weave_camera"
...
if [[ -f $CONTAINER_INIT_DIR/start-dapplications.sh ]]; then
  ...
  docker run --privileged -d --rm --net weave --ip $MYSELF_CONTAINER_IP \
         ...
         --name=$CONTAINER_NAME   \
         jacknlliu/ros:kinetic-ide-init /home/ros/container-scripts/start-dapplications.sh $ROS_MASTER_URI $MYSELF_CONTAINER_IP
else
