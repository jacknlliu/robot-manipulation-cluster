...
LOCAL_HOST_IP="210.72.140.56"

PARTNER_IP_ADDRs="210.72.140.56"

MYSELF_CONTAINER_IP="10.2.2.6"

ROS_MASTER_URI="http://10.2.2.2:11311"

ROBOT_IP_ADDR="210.72.140.56"

DATA_SHARE_DIR="/home/jack/Workspace"
GAZEBO_MODEL_DIR="/home/jack/Workspace/gazebo_models"

CONTAINER_INIT_DIR=$(readlink -f "$0" |xargs dirname)

WEAVE_IP_RANGE="10.2.2.0/24"

CONTAINER_NAME="ros_kinetic_weave_drl"
...
if [[ -f $CONTAINER_INIT_DIR/start-dapplications.sh ]]; then
  ...
  docker run --privileged -it --rm --net weave --ip $MYSELF_CONTAINER_IP \
         --security-opt label=disable   --security-opt seccomp=unconfined \
         --env="DISPLAY" --env QT_X11_NO_MITSHM=1 \
         --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" \
         --volume="$DATA_SHARE_DIR:/home/ros/data:rw" \
